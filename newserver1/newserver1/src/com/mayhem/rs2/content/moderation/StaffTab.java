...
public class StaffTab {
	...
	public static boolean createTarget(Player player, String name, int... rights) {
		...
		if (target == null) {
			...
			try {
				if (!PlayerContainer.loadDetails(target)) {
					player.send(new SendMessage("[ <col=255>GrandLegion</col> ] Player <col=255>" + name + "</col> does not exist!"));
					return false;
				}
			} catch (Exception e) {
				e.printStackTrace();
		...
		if (PlayerConstants.isHighClass(target) || target.getUsername().equalsIgnoreCase("Brandon")) {
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You may not punish <col=255>" + name + "</col>!"));			
			return false;
		}// ??
		...
		if (!access) {
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You do not have access to this!"));	
			return false;
		}
	...
	public static void handle(Player player, String user, PunishmentType punishment, int... rights) {
		...
		case CHECK_BANK:
			...
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You are now viewing <col=255>" + name + "</col>'s bank!"));			
			break;
			
		case KICK:
			target.logout(true);
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully kicked <col=255>" + name + "</col>!"));
			break;
		...
		case UNBAN:
			...
			target.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have been unbanned!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully unbanned <col=255>" + name + "</col>!"));				
			break;
		...
		case JAIL:
			...
			target.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have been jailed for 1 hour!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully jailed <col=255>" + name + "</col> for 1 hour!"));				
			break;
		...
		case UNJAIL:
			...
			target.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have been unjailed!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully unjailed <col=255>" + name + "</col>!"));				
			break;
			
		case MOVE_HOME:
			target.teleport(PlayerConstants.HOME);
			target.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have been moved home!"));
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully moved <col=255>" + name + "</col>!"));							
			break;
		...
		case COPY:
    		...
    		player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully copied <col=255>" + name + "</col>!"));							 		
			break;
			
		case FREEZE:
			target.freeze(10, 5);
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have successfully froze <col=255>" + name + "</col>!"));							 				
			break;
		...
		case INFO:
			...
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You are now viewing <col=255>" + name + "</col>'s account info!"));							 						
			break;
		...
		case GIVE_MODERATOR:
			...
			target.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have been given moderator status by " + player.getUsername() + "!"));
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have given <col=255>" + name + "</col> moderator status."));							 								
			break;
			
		case KILL:
			target.hit(new Hit(target.getSkill().getLevels()[3], HitTypes.DISEASE));
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have killed <col=255>" + name + "</col>!"));							 									
			break;
			
		case TELETO:
			player.teleport(target.getLocation());
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have teleported to <col=255>" + name + "</col>!"));							 									
			break;
			
		case TELETOME:
			target.teleport(player.getLocation());
			target.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have been teleported to <col=255>" + player.getUsername() + "</col>!"));							 											
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have teleported <col=255>" + name + "</col> to your location!"));							 									
			break;
			
		case BOO:
			target.send(new SendInterface(18681));
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have boo'd <col=255>" + name + "</col>!"));							 											
			break;
		...
		case RANDOM_NPC:
			...
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have transformed <col=255>" + name + "</col> into NPC <col=255>" + npcDef.getName() + " </col>(<col=255>" + npcDef.getId() + "</col>)!"));							 											
			break;
		...
		case REFRESH:
			...
			player.send(new SendMessage("[ <col=255>GrandLegion</col> ] You have refreshed <col=255>" + name + "</col>!"));							 											
			break;
