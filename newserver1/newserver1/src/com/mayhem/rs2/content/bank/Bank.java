...
public class Bank extends ItemContainer {
	...
	public Bank(Player player) {
		super(player, SIZE, ItemContainer.ContainerTypes.ALWAYS_STACK, true, false);
		this.player = player;
	}
	...
	public boolean clickButton(int buttonId) {
		...
		case 195087: // option
			setSearching(false);
			if (player.getInterfaceManager().getMain() == 32500) {
		...
		case 195085:
			...
			if (k.length > 1) {
				player.getClient().queueOutgoingPacket(new SendMessage("You deposit your inventory."));
			}
			return true;
		...
		case 195086:
			...
			update();
			if (e.length > 1) {
				player.getClient().queueOutgoingPacket(new SendMessage("You deposit your worn-items."));
			}
	...
	public void deposit(int id, int amount, int slot) {
		...
		if (!hasSpaceFor(new Item(id))) {
			player.send(new SendMessage("@red@You don't have enough space to bank this item!"));
			return;
		}
	...
	public void withdraw(int item, int amount) {
		...
		if (added > 0) {
			if (remove(new Item(old, added), true) > 0) {
				if (added == bankAmount && !player.isPlaceholders()) {
					int tab = getData(bankSlot, 0);
					changeTabAmount(tab, -1, tabAmounts[tab] == 1);
				}
			}
