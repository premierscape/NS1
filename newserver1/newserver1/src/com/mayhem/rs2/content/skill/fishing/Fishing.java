...
public class Fishing {
	...
	public static enum FishingSpots {
		...
		LURE_OR_BAIT(1526, new FishableData.Fishable[] { FishableData.Fishable.ANGLERFISH, FishableData.Fishable.SALMON }, new FishableData.Fishable[] { FishableData.Fishable.SARDINE, FishableData.Fishable.HERRING, FishableData.Fishable.PIKE }),
	...
	public static boolean hasFishingItems(Player player, FishableData.Fishable fish, boolean message) {
		...
		if(tool == 311 || tool == 21031 || tool == 21028) {
			if (!player.getInventory().hasItemAmount(new Item(tool, 1))) {
				...
				if ((weapon != null) && (weapon.getId() == 21031)) {
					return true;
				}
				if ((weapon != null) && (weapon.getId() == 21028)) {
					return true;
				}
				if (message) {
	...
	public boolean fish() {
		...
		if (success(f)) {
			...
			player.sendMessage("You recieve skill points");
			handleRandom(player);
