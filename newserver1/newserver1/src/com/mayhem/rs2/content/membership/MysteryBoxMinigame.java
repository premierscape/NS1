...
public class MysteryBoxMinigame {
	...
	public static void open(Player player) {
		...
		player.send(new SendString("</col>GrandLegion Credits: @gre@" + Utility.format(player.getCredits()), 17006));
		player.send(new SendString("Mystery Box is a gambling minigame where you can bet @gre@" + CREDITS_REQUIRED + " </col>GrandLegion Credits.", 17008));
	...
	public static boolean can(Player player) {
		...
		if (player.getCredits() < CREDITS_REQUIRED) {
			player.send(new SendMessage(MESSAGE_COLOR + "You do not have enough GrandLegion Credits to do this!"));
			return false;
		}
