...
public enum CreditHandler {
	...
	public static boolean allowed(Player player, CreditPurchase credit, int amount) {
		...
		if (player.getCredits() < amount) {
			DialogueManager.sendStatement(player, "@red@You do not have enough GrandLegion Credits to do this!");
			player.send(new SendMessage("Please visit @red@http://www.premierscape.everythingrs.com/services/store </col>to purchase more GrandLegion Credits!"));
			return false;
		}
	...
	public static void spent(Player player, int amount) {
		player.send(new SendMessage("@blu@You have spent " + amount + " GrandLegion Credits; Remaining: " + player.getCredits() + "."));
		player.getClient().queueOutgoingPacket(new SendString("</col>GrandLegion Credits: @gre@" + Utility.format(player.getCredits()), 52504));	
