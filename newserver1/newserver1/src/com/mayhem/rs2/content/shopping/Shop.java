...
public class Shop extends ItemContainer {
	...
	public static void declare() {
		...
		for (int i = 2; i < stock.length; i++) {
			...
			if ((id != 1635) && (id != 440) && (id != 1519) && (id != 2349) && (id != 249) && (id != 29152)
					&& (id != 2351) && (id != 3095)) {
	...
	public void buy(Player player, int slot, int id, int amount) {
		...
		if (player.ironPlayer()) {
			...
			if (!can) {
				player.send(new SendMessage(
						"<img=" + (player.getRights() - 1) + ">@gry@ Iron players may not purchase this item!"));
				return;
			}
		}
		...
		if (amount > 100000) {
			player.getClient().queueOutgoingPacket(new SendMessage(
					"[" + ShopConstants.COLOUR + "*</col>] You can only buy 100K maximum at a time from these shops."));
			amount = 100000;
		}
		...
		if (get(slot).getAmount() == 0) {
			player.getClient().queueOutgoingPacket(
					new SendMessage("[" + ShopConstants.COLOUR + "*</col>] The shop is out of stock on that item."));
			return;
		}
		...
		if (gold.getAmount() > 0) {
			if (player.isPouchPayment()) {
				if (player.getMoneyPouch() < gold.getAmount()) {
					player.send(new SendMessage("[" + ShopConstants.COLOUR
							+ "*</col>] You do not have enough coins in your pouch to do this!"));
					return;
				}
			} else {
				if (!player.getInventory().hasItemAmount(gold)) {
					player.getClient().queueOutgoingPacket(new SendMessage(
							"[" + ShopConstants.COLOUR + "*</col>] You do not have enough coins to buy that."));
					return;
				}
			}
		}
	...
	public boolean sell(Player player, int id, int amount) {
		if (id == 995) {
			player.getClient().queueOutgoingPacket(
					new SendMessage("[" + ShopConstants.COLOUR + "*</col>] You cannot sell coins to a shop."));
			return false;
		}
		for (int i = 0; i < ShopConstants.NO_SELL.length; i++) {
			if (id == ShopConstants.NO_SELL[i]) {
				player.getClient().queueOutgoingPacket(new SendMessage("[" + ShopConstants.COLOUR
						+ "*</col>] You may not sell this item! Try selling it to a player."));
				return false;
			}
		}
		...
		if ((this.store == 21) || ((!general) && (!isDefaultItem(id)))) {
			player.getClient().queueOutgoingPacket(
					new SendMessage("[" + ShopConstants.COLOUR + "*</col>] You cannot sell this item to this shop."));
			return false;
		}

		if (amount > 100000) {
			player.getClient().queueOutgoingPacket(new SendMessage(
					"[" + ShopConstants.COLOUR + "*</col>] You can only sell 100K at a time to these shops."));
			amount = 10000;
		}

		if (GameDefinitionLoader.getItemDef(id).getGeneralPrice() > 10_000_000) {
			player.getClient().queueOutgoingPacket(new SendMessage(
					"[" + ShopConstants.COLOUR + "*</col>] You cannot sell items worth more than 10m!"));
			return false;
		}
		...
		if (!hasSpaceFor(item)) {
			player.getClient().queueOutgoingPacket(new SendMessage(
					"[" + ShopConstants.COLOUR + "*</col>] The shop does not have enough space to buy this item."));
			return false;
		}
		...
		if (!player.getInventory().hasSpaceOnRemove(item, gold)) {
			player.getClient().queueOutgoingPacket(new SendMessage(
					"[" + ShopConstants.COLOUR + "*</col>] You do not have enough inventory space to sell this item."));
			return false;
		}
