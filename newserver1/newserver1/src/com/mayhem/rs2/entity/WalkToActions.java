...
public class WalkToActions {
	 ...
	 */
	// public final static String[] DEFAULT_DIALOGUES = { "When you feel sad,
	// just sing Roar - by Katy Perry!", "Thank the Valius gods for this
	// beautiful day!", "Break free of your cage and be happy!", "Porn is the
	// key to a happy marriage.", "I'm in the mood to groove it out!", "Don't
	// let fear hold you down.", "Just let it out, let it out.", "Make your
	// dreams a reality.", "Be your own creator.", "Divine blesses your soul."
	// };
	...
	public static void clickObject(final Player player, final int option, final int id, final int x, final int y) {
		...
		case 31558:// rev exit
			...
			break;
		case 535:// thermo entrace
			if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
				player.teleport(new Location(2376, 9452));
			}
			break;
		case 536:// thermo exit
			if (player.getLocation().getY() >= y - 3 && player.getLocation().getY() <= y + 3) {
				player.teleport(new Location(2379, 9452));
			}
			break;
		case 29241:
			...
			player.getClient().queueOutgoingPacket(
					new SendMessage("Your Hitpoints, Prayer, Run, and Special have been restored."));
		...
		/*
		 * if ((id == 1815) && x == 3098 && y == 3499 && player.getX() == x &&
		 * player.getY() == y) { player.start(new WildernessLever(player));
		 * return; } if ((id == 1815)) { TaskQueue.queue(new
		 * PullLeverTask(player, x, y, length[0], length[1]) {
		 * 
		 * @Override public void onDestination() {
		 * player.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x +
		 * length[0] / 2 : x, length[1] >= 2 ? y + length[1] / 2 : y);
		 * player.getMagic().teleportNoWildernessRequirement(2561, 3311, 0,
		 * MagicSkill.TeleportTypes.SPELL_BOOK); } }); return;
		 */
		if ((id == 1814)) {
			...
			TaskQueue.queue(new PullLeverTask(player, x, y, length[0], length[1]) {
				...
				public void onDestination() {
					player.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x,
							length[1] >= 2 ? y + length[1] / 2 : y);
					player.getMagic().teleportNoWildernessRequirement(3105, 3951, 0,
							MagicSkill.TeleportTypes.SPELL_BOOK);
				}
			});
			return;
			...
			TaskQueue.queue(new PullLeverTask(player, x, y, length[0], length[1]) {
				...
				public void onDestination() {
					player.getUpdateFlags().sendFaceToDirection(length[0] >= 2 ? x + length[0] / 2 : x,
							length[1] >= 2 ? y + length[1] / 2 : y);
					player.getMagic().teleportNoWildernessRequirement(3105, 3956, 0,
							MagicSkill.TeleportTypes.SPELL_BOOK);
				}
			});
			return;
	...
	public static final void finishClickNpc(Player player, int option, Mob mob) {
		...
		case 1:// NPC FIRST CLICK
			...
			case 4160:// boss points
				...
				player.getClient()
						.queueOutgoingPacket(new SendMessage("@blu@ You have " + player.bossPoints + " Boss points."));
				break;
			case 6781:// Skill points
				player.getShopping().open(SkillPointShop.SHOP_ID);
				player.getClient().queueOutgoingPacket(
						new SendMessage("@blu@ You have " + player.skillPoints + " Skill points."));
				break;
			case 5802:// pvm points
				player.getShopping().open(PvmShop.SHOP_ID);
				player.getClient().queueOutgoingPacket(
						new SendMessage("@blu@ You have " + player.pvmPoints + " Player vs Monster(PVM) points."));
				break;
			...
			case 822:// Oziach
				...
				break;
			case 954:// Barrows
				DialogueManager.sendNpcChat(player, 954, Emotion.HAPPY, "Oy! Need your items repaired?",
						"I can fix them for 250,000 coins each.", "Simply use your item on me.");
				break;
			case 5787:// Agility
			...
			case 7663:
				player.start(new OptionDialogue("Obtain a task.", p -> {
					if (player.getSlayer().hasTask()) {
						DialogueManager.sendNpcChat(player, 7663, Emotion.ANGRY_1, "You already have a task!",
								"Complete your current task or please reset it...");
						return;
					}
					player.getSlayer().assign(Slayer.SlayerDifficulty.WILDERNESS);
					DialogueManager.sendNpcChat(player, 7663, Emotion.HAPPY, "You have to kill "
							+ player.getSlayer().getAmount() + " " + player.getSlayer().getTask() + "'s.");
				}, "View Rewards", p -> {
					player.getShopping().open(6);
				}, "Reset my task (@red@250k@bla@)", p -> {
					if (!player.getSlayer().hasTask()) {
						DialogueManager.sendNpcChat(player, 7663, Emotion.ANGRY_1,
								"You do not even have a slayer task!");
						return;
					} else if (!player.getInventory().hasItemAmount(995, 250000)) {
						DialogueManager.sendNpcChat(player, 7663, Emotion.ANGRY_1,
								"You do not even have 250,000 coins!");
						return;
					} else {
						player.getSlayer().reset();
						DialogueManager.sendStatement(player,
								new String[] { "You have reset your task for @blu@250,000 </col>coins." });
			...
			case 1771:// Novice squire
				DialogueManager.sendNpcChat(player, 1771, Emotion.HAPPY,
						"Welcome to Pest Control " + player.getUsername() + "!");
				break;
			...
			default:
				...
				// DialogueManager.sendNpcChat(player, id, Emotion.DEFAULT,
				// Utility.randomElement(DEFAULT_DIALOGUES));
				break;
			}
			break;
		...
		case 2:// NPC SECOND CLICK
			...
			case 6875:
				if (PlayerConstants.isPlayer(player)) {
					DialogueManager.sendNpcChat(player, 6875, Emotion.DEFAULT,
					...
					player.getClient()
							.queueOutgoingPacket(new SendMessage("Your Hitpoints and Prayer are already full."));
				}
				break;
			case 2195:// William
			...
			case 4249:// Achievement
				...
				player.getClient().queueOutgoingPacket(
						new SendMessage("@blu@ You have " + player.getAchievementsPoints() + " Achievement points."));
				break;
			case 1603:// Mage Arena
			...
			case 2132:
				...
				player.start(new OptionDialogue((String) dialogues.get(0)[0], (Consumer<Player>) (dialogues.get(0)[1]),
						(String) dialogues.get(1)[0], (Consumer<Player>) (dialogues.get(1)[1])));
				break;
			case 822:// Oziach
			...
			case 5787:// Agility
				...
				break;
			case 7042:// Swagster
				//player.start(new OptionDialogue("GrandLegion Credits Store 1", p -> {
					//player.getShopping().open(94);
				//}, "GrandLegion Credits Store 2", p -> {
				//	player.getShopping().open(90);
				//}, "GrandLegion Credits Store 3", p -> {
			...
			case 306:// tutorial npc
				...
				}, "Server tour(Show several areas around GrandLegion)", p -> {
			...
			case 3215:// Prestige
				...
				player.getClient().queueOutgoingPacket(
						new SendMessage("@blu@ You have " + player.prestigePoints + " Prestige points."));
				break;
			case 395:// Banker
				player.getBank().openBank();
				break;
		...
		case 3:// NPC THIRD CLICK
			...
			case 1103:// Weapon Game
				...
				player.getClient().queueOutgoingPacket(
						new SendMessage("@blu@ You have " + player.weaponPoints + " Weapon points."));
				break;
			case 1306:// Make over mage
			...
			case 7042:// Swagster
				if (PlayerConstants.isPlayer(player)) {
					DialogueManager.sendNpcChat(player, 7042, Emotion.DEFAULT,
							"You need to be a <img=4>@red@member </col>to do this!");
					return;
				}
			...
			case 2150:// Exchange agent
				...
				player.getClient()
						.queueOutgoingPacket(new SendString("Shops Exchange | @red@0</col> Active Shops", 53505));
			...
			if (Utility.random(200) == 0) {
				...
				if (petDrop != null) {
					if (player.getBossPet() == null) {
						...
						player.send(new SendMessage("You feel a pressence following you; "
								+ Utility.formatPlayerName(
					...
					GroundItemHandler.add(new Item(13178, 1), player.getLocation(), player,
							player.ironPlayer() ? player : null);
				}
			} else {
			...
			for (int[] id : Barrows.BROKEN_BARROWS) {
				if (item == id[1]) {
					if (player.getInventory().contains(new Item(995, 250_000))) {
						...
						DialogueManager.sendNpcChat(player, 954, Emotion.DEFAULT,
								"Your " + GameDefinitionLoader.getItemDef(id[0]).getName() + " has been repaired.");
					} else {
						DialogueManager.sendNpcChat(player, 954, Emotion.EVIL_LAUGH_SHORT,
	...
	public static void finishObjectClick(Player player, int id, int option, int x, int y) {
		...
		if (PlayerConstants.isOwner(player)) {
			...
			if (o != null) {
				player.getClient().queueOutgoingPacket(new SendMessage("Object option: " + option + " id: " + id
						+ " x: " + x + " y: " + y + " face: " + o.getFace() + " type: " + o.getType()));
			} else {
		...
		if ((id == 3641) && (x == 1730) && (y == 3492) && player.isExtreme()) {
			if (player.getSkill().getLevels()[Skills.PRAYER] < player.getMaxLevels()[Skills.PRAYER]) {
				...
				player.getClient()
						.queueOutgoingPacket(new SendMessage("Your Hitpoints and Prayer are already restored."));
			}
		}
		...
		case 1:
			...
			if (Agility.SINGLETON.fireObjectClick(player, player.getLocation(),
					Region.getObject(x, y, player.getLocation().getZ()))) {
				return;
			}
			...
			if (ObjectDef.getObjectDef(id) != null && ObjectDef.getObjectDef(id).name != null
			...
			case 29170:// max cape rack
				...
				DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL, "You'll have to talk to me",
						"about getting your first @blu@Max cape</col>.");
				break;
			...
			case 29171:// fire max cape rack
				if (!player.getInventory().hasItemAmount(13280, 1) && !player.getInventory().hasItemAmount(13281, 1)
					...
					DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL2,
							"You're missing some items. You'll need a:", "@blu@Max hood</col>", "@blu@Max cape</col>",
							"@blu@Firecape</col>.");
				...
				if (player.getInventory().hasItemAmount(13280, 1) && player.getInventory().hasItemAmount(13281, 1)
					...
					DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL2, "Enjoy your Fire max cape.",
							"It has the bonuses of a Firecape!");
				}
				break;
			...
			case 29175:// avas max cape rack
				...
				if (player.getInventory().hasItemAmount(13280, 1) && player.getInventory().hasItemAmount(13281, 1)
					...
					DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL2, "Enjoy your Ava's max cape.",
							"It collects arrows like the accumulator!");
				}
				break;
			...
			case 29174:// guthix max cape rack
				...
				if (player.getInventory().hasItemAmount(13280, 1) && player.getInventory().hasItemAmount(13281, 1)
					...
					DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL2, "Enjoy your Guthix max cape.",
							"It has gives 25% extra damage to all Magic spells!");
				}
				break;
			...
			case 29173:// Zamorak max cape rack
				...
				if (player.getInventory().hasItemAmount(13280, 1) && player.getInventory().hasItemAmount(13281, 1)
					...
					DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL2, "Enjoy your Zamorak max cape.",
							"It has gives 25% extra damage to all Magic spells!");
				}
				break;
			case 29172:// Saradomin max cape rack
				...
				if (player.getInventory().hasItemAmount(13280, 1) && player.getInventory().hasItemAmount(13281, 1)
					...
					DialogueManager.sendNpcChat(player, 6481, DELIGHTED_EVIL2, "Enjoy your Saradomin max cape.",
							"It has gives 25% extra damage to all Magic spells!");
				}
				break;
			...
			case 26762:// Scorpion Pit
				...
				TaskQueue.queue(new Task(player, 1, false) {
					...
					@Override
					public void onStop() {
						if (player.getLocation().equals(new Location(3233, 3938))
								|| player.getLocation().equals(new Location(3232, 3938))) {
							player.teleport(new Location(3233, 10332));
						} else if (player.getLocation().equals(new Location(3233, 3950))
								|| player.getLocation().equals(new Location(3232, 3950))) {
							player.teleport(new Location(3232, 10351));
						} else if (player.getLocation().equals(new Location(3242, 3948))
								|| player.getLocation().equals(new Location(3243, 3948))) {
			...
			case 14844:
			case 14845:
			case 14847:// abyss portals
				player.send(new SendMessage("You tele back to the abyss."));
				player.teleport(new Location(3039, 4834));
				break;
			...
			case 18987:// KBD ladder
				...
				break;
			case 26761:// wildy in lever
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
					player.teleport(new Location(3154, 3923, 0));
				}
				break;
			case 1815:// wildy out lever
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
			...
			case 28857:// redwood up ladders
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
					player.teleport(new Location(1570, 3488, 1));
				}
				break;
			case 31557:// wildy exit
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
					player.teleport(new Location(3126, 3833, 0));
				}
			case 26738:// rev slay entrance
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
					player.teleport(new Location(3196, 10056, 0));
				}
				break;
			case 537:// Kraken Entrance
				player.teleport(new Location(2279, 10022, player.getIndex() << 2));
				TaskQueue.queue(new Task(5) {
					@Override
					public void execute() {
						Kraken mob = new Kraken(player, new Location(2279, 10036, player.getIndex() << 2));
						player.send(new SendMessage("Welcome to Kraken's Cave."));
						DialogueManager.sendStatement(player, "Welcome to Kraken's Cave.");
						stop();
					}
				...
				break;
			case 538:// Kraken exit
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
					player.teleport(new Location(2279, 10016, 1796));
				}
				break;

			case 10068:// Zulrah tele
			...
			case 7527: // Stile fally cow farm
				if (player.getLocation().equals(new Location(3045, 3305, 0))) {
					...
					player.getUpdateFlags()
							.sendFaceToDirection(new Location(player.getX() - 3, player.getY(), player.getZ()));
			...
			case 28858:// redwood down ladders
				if (player.getLocation().getX() >= x - 3 && player.getLocation().getX() <= x + 3) {
					player.teleport(new Location(1579, 3487));
				}
				break;
			...
			case 2557:
			case 2558:
				if (player.getX() == 3038 && player.getY() == 3956 || player.getX() == 3044 && player.getY() == 3956
						|| player.getX() == 3041 && player.getY() == 3959
						|| player.getX() == 3190 && player.getY() == 3957
			...
			case 6552:// ancient altar
				player.getMagic().setSpellBookType(player.getMagic().getSpellBookType() == SpellBookTypes.MODERN
						? SpellBookTypes.ANCIENT : SpellBookTypes.MODERN);
				player.getMagic()
			...
			case 410:// lunar altar
				player.getMagic().setSpellBookType(player.getMagic().getSpellBookType() == SpellBookTypes.MODERN
						? SpellBookTypes.LUNAR : SpellBookTypes.MODERN);
				player.getMagic()
		...
		case 3:
			...
			case 26813:
				DialogueManager.sendStatement(player,
						"You have @blu@" + Utility.format(player.getVotePoints()) + " </col>voting points.");
				break;
			case 9371:
			case 9472:// Exchange
			case 9741:
	...
	public static void itemOnObject(final Player player, final int itemId, final int objectId, final int x,
			final int y) {
		if (Constants.DEV_MODE) {
				...
				if (objectId == 6943) {
					...
					if (def.getNoteId() == -1) {
						player.send(new SendMessage(
								def.getName() + " cannot be " + (def.isNote() ? "un-noted" : "noted") + "."));
						return;
					}
