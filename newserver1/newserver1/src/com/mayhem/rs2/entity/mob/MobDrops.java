	...
	public static void drop(Entity entity, Mob mob, ItemDropDefinition.ItemDropTable table, Location dropLocation) {
		if ((table != null) && (table.getDrops() != null)) {
			...
			if (entity.isNpc()) {
				...
				} else if(item.getDefinition().getGeneralPrice() >= 100_000) {
			...
			if (roll >= 500 && roll <= 500 + chance + mod) {
				...
				if (p != null) {
					...
					p.getRareDropEP().addReceived();
						...
						if(item.getDefinition().getGeneralPrice()> 1_000_000){
						World.sendGlobalMessage("<col=1F8C26>" + e.getPlayer().getUsername() + " recieved a drop: " + Utility.format(item.getAmount()) + " x " + item.getDefinition().getName() + "." + " kill count: @red@" + Utility.format(e.getPlayer().getProperties().getPropertyValue("MOB_" + mob.getDefinition().getName())));
						}else{
						//World.sendRegionMessage("<col=1F8C26>" + e.getPlayer().getUsername() + " recieved a drop: " + Utility.format(item.getAmount()) + " x " + item.getDefinition().getName() + ".",e.getLocation());
						}
				}
