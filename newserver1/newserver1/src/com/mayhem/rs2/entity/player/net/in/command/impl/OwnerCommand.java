...
import com.mayhem.rs2.content.event.impl.DoubleDropRate;
	...
	public boolean handleCommand(Player player, CommandParser parser) throws Exception {
		...
		case "givemod":
			...
			case "givesupport":
				if (parser.hasNext()) {
					...
					World.sendGlobalMessage("</col>[ @dre@GrandLegion </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + " has just been promoted to Player Support! ");
		...
		case "giveowner":
			if (parser.hasNext()) {
				...
				p.setRights(3);
				p.send(new SendMessage("You have been given owner status by " + player.determineIcon(player) + " " + player.getUsername()));
				player.send(new SendMessage("You have given owner status to: @red@" + p.getUsername()));
			}
			return true;
		/*
		...
		case "doubledrop":
			World.getEventManager().setEvent (new DoubleDropRate());
			return true;
