...
import com.mayhem.rs2.entity.player.net.out.impl.SendGlobalSound;
...
public class PlayerCommand implements Command {
	...
	public boolean handleCommand(Player player, CommandParser parser) throws Exception {
		...
		case "song":
			player.send(new SendSong(177));
		return true;
		
		case "sound":
			player.send(new SendSound(334,100,5));
		return true;
		
		case "globalsound":
			player.send(new SendGlobalSound(10,2,2));
		return true;
			
			
			
		case "droprate":
			player.send(new SendMessage("Your Rare Drop EP: " + player.getRareDropEP()));
			return true;
		...
		case "reward":
			...
			com.everythingrs.vote.Vote.service.execute(new Runnable() {
				...
				public void run() {
					try {
						com.everythingrs.vote.Vote[] reward = com.everythingrs.vote.Vote.reward("", playerName, id, rewardAmount);
		...
		case "commandlist":
		case "commandslist":
			player.send(new SendString("GrandLegion Command List", 8144));
		...
		case "stats":
		case "highscores":
			player.send(new SendString("http://www.premierscape.everythingrs.com/services/hiscores", 12000));
		...
		case "forums":
		case "website":
			player.send(new SendString("http://www.premierscape.net", 12000));
			 ...
			 */
			case "discord":
				player.send(new SendString("https://discord.gg/mukUBEP", 12000));
		...
		case "changepass":
             ...
             */
		case "bank":
			if (player.getRights() == 1 || player.getRights() == 2 ||  player.getRights() == 3 ||  player.getRights() == 4 || player.getRights() == 8 || player.getRights() == 13 || player.getMoneySpent() >= 50) {
		...
		case "home":
			...
			player.getMagic().teleport(3088, 3490, 0, TeleportTypes.SPELL_BOOK);
			return true;
