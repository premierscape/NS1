...
public class Player extends Entity {
	...
	public void setPlaceholders(boolean placeholders) {
		...
		this.send(new SendConfig(118, placeholders ? 1 : 0));
	}
...
private MysterySpin donorBox= new MysterySpin(this);
	...
	public boolean login(boolean starter) throws Exception {
		...
		send(new SendString("</col>GrandLegion Credits: @gre@" + Utility.format(this.getCredits()), 52504));
		...
		send(new SendMessage("<img=7>@blu@Welcome to GrandLegion!"));
		send(new SendMessage("<img=7>@blu@Make sure you register on our active community forums: www.Premierscape.net"));
		send(new SendMessage("<img=7>@blu@Don't forget to ::Vote every 12 hours for great rewards and to help the server grow!"));
		send(new SendMessage("<img=7>@blu@Type ::Donate to visit our online store!"));
		
		if (World.getActivePlayers() > Constants.MOST_ONLINE) {
			Constants.MOST_ONLINE = World.getActivePlayers();
			FileHandler.saveMaxPlayers();
			World.sendGlobalMessage("[<col=910D0D>GrandLegion</col>] We have broken our most players online record! New record: <col=910D0D>" + Constants.MOST_ONLINE + "</col>!");
		}
	...
	public void logout(boolean force) {
		if (isActive()) {
			if (force){
				...
				com.everythingrs.hiscores.Hiscores.update("npzvkf1degh4hg218115opqfrz8l0i3aidnqmmc6lqx4ezsemio3k3962ep9dpkozgd5p6tj4i",  "Normal Mode", this.getUsername(),
